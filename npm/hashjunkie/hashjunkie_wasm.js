/* @ts-self-types="./hashjunkie_wasm.d.ts" */
...
export class WasmHasher {
    __destroy_into_raw() {
        ...
        this.__wbg_ptr = 0;
        WasmHasherFinalization.unregister(this);
        return ptr;
    }
    free() {
    ...
    finalize() {
        try {
            ...
            wasm.wasmhasher_finalize(retptr, this.__wbg_ptr);
    ...
    constructor(algorithms) {
        try {
            const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
            wasm.wasmhasher_new(retptr, addHeapObject(algorithms));
            var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
            var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
            var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
            if (r2) {
    ...
    update(data) {
        try {
            ...
            wasm.wasmhasher_update(retptr, this.__wbg_ptr, ptr0, len0);
            var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
            var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
            if (r1) {
                throw takeObject(r0);
...
if (Symbol.dispose) WasmHasher.prototype[Symbol.dispose] = WasmHasher.prototype.free;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('hashjunkie_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
